...
func TestBoundedWorkerPool(t *testing.T) {
	t.Run("Basic Functionality", func(t *testing.T) {
		pool := utils.NewBoundedWorkerPool(utils.BoundedWorkerPoolConfig{
			Workers: 2,
			Queue:   10,
			Timeout: time.Second,
		})
...
func TestRateLimiter(t *testing.T) {
	t.Run("Basic Rate Limiting", func(t *testing.T) {
		rl := utils.NewRateLimiter(10, time.Second) // 10 requests per second
	...
	t.Run("Rate Limiting with Time", func(t *testing.T) {
		rl := utils.NewRateLimiter(5, 100*time.Millisecond) // 5 requests per 100ms
...
func TestRateLimiterConcurrency(t *testing.T) {
	rl := utils.NewRateLimiter(100, time.Second)
...
func BenchmarkRateLimiter(b *testing.B) {
	rl := utils.NewRateLimiter(1000000, time.Second)
