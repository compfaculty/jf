...
func (r *SQLiteRepo) UpsertAggregator(ctx context.Context, a *models.Aggregator) error {
	now := time.Now().UTC()
	if a.ID == "" {
		a.ID = uuid.NewString()
	}
...
func (r *SQLiteRepo) UpsertAggregatorByName(ctx context.Context, a *models.Aggregator) error {
	now := time.Now().UTC()
	if a.ID == "" {
		a.ID = uuid.NewString()
	}
...
func (r *SQLiteRepo) ListAggregators(ctx context.Context) ([]models.Aggregator, error) {
	//goland:noinspection SqlResolve
