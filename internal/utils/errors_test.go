...
func TestAppError(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := NewAppError(tt.errType, tt.message, tt.originalErr)
...
func TestSpecificErrorConstructors(t *testing.T) {
	...
	t.Run("NewDatabaseError", func(t *testing.T) {
		err := NewDatabaseError("query failed", errors.New("syntax error"))
	...
	t.Run("NewParsingError", func(t *testing.T) {
		err := NewParsingError("invalid format", errors.New("unexpected token"))
...
func TestAppErrorUnwrap(t *testing.T) {
	...
	appErr := NewAppError(ErrorTypeNetwork, "wrapped error", originalErr)
	...
	appErr2 := NewAppError(ErrorTypeNetwork, "no original error", nil)
...
func TestErrorTypeString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.expected, func(t *testing.T) {
			...
			if tt.errType < ErrorTypeUnknown || tt.errType > ErrorTypeValidation {
...
func TestAppErrorFields(t *testing.T) {
	err := NewAppError(ErrorTypeNetwork, "test error", errors.New("original"))
...
func TestAppErrorStackTrace(t *testing.T) {
	err := NewAppError(ErrorTypeNetwork, "test error", nil)
...
func TestAppErrorErrorMethod(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			err:      NewAppError(ErrorTypeNetwork, "network failed", errors.New("timeout")),
			expected: "network failed: timeout",
		},
		{
			name:     "without original error",
			err:      NewAppError(ErrorTypeDatabase, "database failed", nil),
...
func TestErrorTypeConstants(t *testing.T) {
	...
	if ErrorTypeUnknown != 0 {
		t.Errorf("Expected ErrorTypeUnknown to be 0, got %d", ErrorTypeUnknown)
	}

	if ErrorTypeNetwork != 1 {
		t.Errorf("Expected ErrorTypeNetwork to be 1, got %d", ErrorTypeNetwork)
	}

	if ErrorTypeDatabase != 2 {
		t.Errorf("Expected ErrorTypeDatabase to be 2, got %d", ErrorTypeDatabase)
	}

	if ErrorTypeParsing != 3 {
		t.Errorf("Expected ErrorTypeParsing to be 3, got %d", ErrorTypeParsing)
	}

	if ErrorTypeValidation != 4 {
		t.Errorf("Expected ErrorTypeValidation to be 4, got %d", ErrorTypeValidation)
	}
}
...
func BenchmarkAppErrorCreation(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = NewAppError(ErrorTypeNetwork, "test error", errors.New("original error"))
	}
}

func BenchmarkAppErrorString(b *testing.B) {
	err := NewAppError(ErrorTypeNetwork, "test error", errors.New("original error"))
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = err.Error()
	}
