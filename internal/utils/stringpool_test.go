...
func TestOptimizedStringBuilder(t *testing.T) {
	tests := []struct {
		name       string
		capacity   int
		operations []func(*OptimizedStringBuilder)
		expected   string
	}{
		{
			name:       "empty",
		...
		},
		{
			...
			capacity: 10,
			operations: []func(*OptimizedStringBuilder){
				func(sb *OptimizedStringBuilder) { sb.WriteString("hello") },
			},
			expected: "hello",
		},
		{
			...
			operations: []func(*OptimizedStringBuilder){
				func(sb *OptimizedStringBuilder) { sb.WriteString("hello") },
				func(sb *OptimizedStringBuilder) { sb.WriteRune(' ') },
				func(sb *OptimizedStringBuilder) { sb.WriteString("world") },
				func(sb *OptimizedStringBuilder) { sb.WriteByte('!') },
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			sb := NewOptimizedStringBuilder(tt.capacity)
...
func TestOptimizedStringBuilderReset(t *testing.T) {
	sb := NewOptimizedStringBuilder(10)
	sb.WriteString("test")
...
func TestOptimizedJoin(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := OptimizedJoin(tt.sep, tt.elems)
			if result != tt.expected {
...
func BenchmarkOptimizedStringBuilder(b *testing.B) {
	for i := 0; i < b.N; i++ {
		sb := NewOptimizedStringBuilder(100)
